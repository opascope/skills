...
| **Search term buyer intent** (buyer, researcher, job seeker, wrong product) | **Jev** | meaning of a query; one practitioner ran 12,418 terms in 12 seconds |
...
| **Ad to landing page match** (does the page keep the ad's promise) | **Jev** | the most corroborated new idea in the market, 27 independent proposals |
...
| **Creative grading** (hook type, claim risk, CTA, funnel stage) | **Jev** | see creative.md |
...
| Move bids or budgets | code, gated on confidence | "If the model isn't sure, it doesn't move money" |
| Write new ad copy | writing model | Jev cannot write |

## Ready jobs

- `jobs/paid-search-term-intent.json`: buyer intent per search term. Input needs a
  `search_term` column; describe what you sell in `shared_state.offer`, since "buyer" only
  means something relative to the offer.
- `jobs/paid-ad-landing-match.json`: how well the page keeps the ad's promise, and the fix.
...
- **Acting on low confidence with money.** Every production pattern we found gates spend
  changes on confidence and sends the rest to a person.
