...
| **Does the page keep the ad's promise?** | **Jev**, score | the market's top new use case; route to keep, rewrite the ad, or rewrite the page |
...
- `jobs/paid-ad-landing-match.json`: promise match score plus the suggested fix. Input needs
  `ad_headline`, `ad_text`, `landing_page_text`.
- `jobs/landing-objections.json`: whether the page answers three common objections. Edit the
