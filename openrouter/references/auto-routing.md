...
Seven calls through the router, from the script in this folder. Costs are what each
...
- **A tiny budget can come back empty.** With 16 tokens the reply was empty with finish
  reason `length`. The router refuses a request that turns reasoning off (HTTP 400). So
...
Total spend for these probes was $0.00054.
